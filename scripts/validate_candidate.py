"""
validate_candidate.py - physics validation for the CANDIDATE dict below.

Edit CANDIDATE to test any geometry. Reads baseline.yaml for material/limits.
Usage: python scripts/validate_candidate.py
"""
...
# edit CANDIDATE to test any geometry
CANDIDATE = {
    ...
    'd_shaft_A':      0.003000,  # motor output shaft diameter (m)
...
def _kinematic_check(d: dict) -> dict:
    # checks that l > r + e so the mechanism can complete a full rotation
    r, l, e = d['r'], d['l'], d['e']
    margin = l - (r + e)
...
def _net_section_check(d: dict, min_net: float) -> dict:
    # returns how much material is left at each pin hole (positive = OK)
    pairs = {
...
def run() -> None:
    config = load_config(str(_CFG_PATH))
    ...
    # see instructions.md Section 3.4 for derivation
    ...
    ### Pre-check: kinematic feasibility
    ...
    if not kin_check['feasible']:
        ...
        print()
        print("  The mechanism cannot complete a full rotation because the rod is")
        print("  too short to reach pin C at some crank angles (around 75 deg).")
        print("  The optimizer found this because it treats r, l, e independently,")
        print("  but training data only had designs where l > r + e.")
        print("=" * W)
    ...
    print(f"    volume_envelope: {design_eval['volume_envelope']*1e6:>8.2f} cm^3")
