...
- Gravity distributed bending **removed**: rod self-weight already in F_B, F_C via Newton-Euler — separate UDL double-counts it
- OOP moment `M_eta = F_r_rod_B · i_offset` is **constant along rod** — system is fully planar (2D dynamics), no ζ pin reactions exist to create a lateral force couple → no linear decay
...
- Torsion: `T_rod = |F_t_C| · i_offset` (F_t at slider end; Eq 5.6)
