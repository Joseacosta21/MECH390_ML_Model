...
# Basquin S-N second anchor point (Mother Doc Eq 13.3 two-point method).
_N_BASQUIN_ANCHOR: float = 2.0e6   # cycles
...
def _k_e() -> float:
    """
    Reliability factor at 99.9% reliability (Mother Doc Section 10.2).

    k_e = 1 - 0.08 * z_a  with z_a = 3.091 for 99.9%.
    Recommended for fatigue-critical aluminium components.
    """
    return 1.0 - 0.08 * 3.091   # ≈ 0.753
...
def _endurance_limit_rect(
    ...
    Returns:
        S_e (Pa)
    """
    return _k_a(S_ut) * _k_b_rect(w, t) * _k_c() * _k_d() * _k_e() * _k_f() * S_prime_e
...
def _endurance_limit_pin(
    ...
    Returns:
        S_e (Pa)
    """
    return _k_a(S_ut) * _k_b_pin(d) * _k_c() * _k_d() * _k_e() * _k_f() * S_prime_e
...
def _basquin_exponent(S_e: float, S_ut: float) -> float:
    """
    ...
    Args:
        ...
        S_ut:  ultimate tensile strength (Pa)
    """
    return -math.log10(0.9 * S_ut / S_e) / math.log10(2.0 * _N_BASQUIN_ANCHOR)
...
def _cycles_to_failure(
    ...
    if sigma_a_eq <= 0.0:
        return float('inf')
    b_B = _basquin_exponent(S_e, S_ut)
...
def _component_fatigue(
    ...
    b_B = _basquin_exponent(S_e, S_ut)
    N_f = _cycles_to_failure(sigma_a_eq, S_e, S_ut, sigma_f_prime)
...
def evaluate(
    ...
    S_ut          = float(design.get('S_ut',          483e6))
    S_y           = float(design.get('S_y',           345e6))
    S_prime_e     = float(design.get('S_prime_e',     130e6))
    sigma_f_prime = float(design.get('sigma_f_prime', 807e6))
    n_rpm         = float(design.get('n_rpm',         30.0))
    total_cycles_raw = design.get('total_cycles', None)
    total_cycles: Optional[float] = (
    ...
    S_e_rod   = _endurance_limit_rect(w_rod,   t_rod,   S_ut, S_prime_e)
    S_e_crank = _endurance_limit_rect(w_crank, t_crank, S_ut, S_prime_e)
    # For pins: use smallest pin diameter — most conservative k_b (size factor)
    d_pin_min = min(D_pA, D_pB, D_pC)
    S_e_pin   = _endurance_limit_pin(d_pin_min, S_ut, S_prime_e)
    ...
    rod   = _component_fatigue(
        ...
        S_e_rod,   S_ut, S_y, sigma_f_prime, n_rpm, total_cycles,
    )
    crank = _component_fatigue(
        arr_sig_crank, arr_tau_crank,
        S_e_crank, S_ut, S_y, sigma_f_prime, n_rpm, total_cycles,
    )
    pin   = _component_fatigue(
        arr_sig_pin,   arr_tau_pin,
        S_e_pin,   S_ut, S_y, sigma_f_prime, n_rpm, total_cycles,
    )
