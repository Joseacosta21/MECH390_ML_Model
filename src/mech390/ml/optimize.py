"""
Surrogate-based optimizer for the crank-slider mechanism.

Workflow:
1. Load the trained surrogate checkpoint + scaler + target_stats.
2. Read geometry bounds from baseline.yaml.
3. Read weight table and constraints from search.yaml.
4. Run scipy differential_evolution over the 10-dim geometry space.
5. Return and print the top-N geometries ranked by weighted score.

Score function:
    score(x) = sum(w_i * normalize(obj_i(x)))
               - sum(w_i * ood_penalty(obj_i(x)))
               - penalty * max(0, threshold - pass_prob(x))
               - hard constraint penalties

    normalize maps each objective to [0, 1] using training-set min/max:
      - minimize objectives: contribution = 1 - normalized_value
      - maximize objectives: contribution = normalized_value
    ...
    the objective weight and ood_penalty_scale.

      ood_amount = max(0, -norm - tol) + max(0, norm - 1 - tol)
      penalty = weight * ood_amount * ood_penalty_scale
...
# 10-dim geometry search space; same order as F.RAW_GEO_KEYS
# distinct from F.INPUT_FEATURES (12-dim) which also has derived slenderness features
_GEO_KEYS = F.RAW_GEO_KEYS

# maps objective name to its index in REGRESSION_TARGETS
_OBJ_IDX = {name: i for i, name in enumerate(F.REGRESSION_TARGETS)}


### Bounds extraction

# reads geometry min/max from the generate config and returns them in _GEO_KEYS order
...
def _build_score_fn(
    ...
    """Returns a callable f(x) -> -score that scipy can minimise over the 10-dim geometry space.

    Hard constraints enforced as penalties:
    - pass_prob >= pass_threshold (surrogate gate)
    - width - D_pin > min_net_section for all four pin pairs (prevents near-zero net sections)
    - l > r + e (rod must reach pin C from pin B at all crank angles)
    - Euler buckling FoS >= n_buck_min (analytical: P_cr = pi^2*E*I/l^2)
    - |ROM - rom_target| <= rom_tolerance (stroke must stay at 250 mm +- 0.5 mm)
    - QRR in [qrr_min, qrr_max] (quick-return ratio must stay in training band)

    OOD penalty: predictions normalised outside [-ood_tolerance, 1+ood_tolerance]
    get a score deduction proportional to the excess, weight, and ood_penalty_scale.
    """
    # (width_idx, pin_idx) pairs in _GEO_KEYS for the four net-section checks
    _NET_PAIRS = [
    ...
    # evaluates one geometry vector and returns -score (negative so scipy can minimise)
    def _score(x: np.ndarray) -> float:
        # extend 10-dim raw geometry to 12-dim model input, then scale
        ...
        # model outputs are in [0, 1] - denormalise to physical units before scoring
        ...
        for obj_name, obj_cfg in objectives.items():
            ...
            # OOD penalty: deduct from score if prediction is outside training range
            # applied before clipping so the full excess is captured
        ...
        # ROM and QRR: all training data satisfies ROM = 250 mm +- 0.5 mm and QRR in [1.5, 2.5]
        # without these penalties the optimizer picks arbitrary r values with wrong stroke
        ...
        return -score   # scipy minimises, so return negative score
...
def run_optimization(
    ...
    """Runs differential evolution and returns a list of top-N candidate dicts with scores and predictions."""
    ...
    # penalty scaling factors from search.yaml
    ...
    # records the best score seen at each optimizer iteration
    global_convergence_log = []

    # called by differential_evolution after each iteration to log convergence
    def convergence_callback(xk, convergence=None):
        global_convergence_log.append(float(-score_fn(xk)))
    ...
    # collect top-N by sampling near the best result and re-scoring
    # differential_evolution returns only the single best; sample extras to fill top-N slots
    ...
    # score all candidates, sort descending, deduplicate
    scored = []
    for x in candidates:
        x_full = F.raw_to_model_input(x)
        x_norm = scaler.transform(x_full.reshape(1, -1)).astype(np.float32)
    ...
    # keep only distinct candidates that passed the surrogate gate with a positive score
    ...
    if not unique:
        # fallback: no candidate passed the gate - return best by score anyway
        logger.warning(
...
def find_dataset_best(
    ...
    """Finds the highest-scoring design in passed_configs.csv using the same objective weights as the optimizer.

    Uses actual physics values from the CSV rather than surrogate predictions.
    Returns a result dict in the same format as run_optimization(), or None if the CSV is missing.
    """
    try:
        import pandas as pd
    except ImportError:
    ...
    # scores one CSV row using the same normalised objective formula as the optimizer
